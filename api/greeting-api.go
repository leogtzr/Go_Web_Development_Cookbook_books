...
func (s *Say) Hello(ctx context.Context, rq *api.Request, rsp *api.Response) error {
	...
	if ok {
		response, err := s.Client.Hello(
			ctx, &hello.Request{Name: strings.Join(name.Values, " ")},
		)
		if err != nil {
			return err
		}
		message, _ := json.Marshal(map[string]string{"message": response.Msg})
...
func main() {
	...
	service.Init()
	service.Server().Handle(service.Server().NewHandler(
